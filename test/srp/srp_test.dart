  ...
  group("SRP", () {
    group("rfc5054:", () {
      test('rfc5054AppendixBTestVectors', () {
        Uint8List I = Uint8List.fromList("alice".codeUnits);
        Uint8List P = Uint8List.fromList("password123".codeUnits);
        Uint8List s = hex.decode("BEB25379D1A8581EB5A727673A2441EE");
        BigInt N = SRP6StandardGroups.rfc5054_1024.N;
        BigInt g = SRP6StandardGroups.rfc5054_1024.g;
        ...
        BigInt k = SRP6Util.calculateK(Digest("SHA-1"), N, g);
        if (k.compareTo(expect_k) != 0) {
          fail("wrong value of 'k', expected $expect_k got $k");
        }

        BigInt x = SRP6Util.calculateX(Digest("SHA-1"), N, s, I, P);
        if (x.compareTo(expect_x) != 0) {
          fail("wrong value of 'x'");
        }

        SRP6VerifierGenerator gen = SRP6VerifierGenerator(
            group: SRP6StandardGroups.rfc5054_1024, digest: Digest("SHA-1"));
        BigInt v = gen.generateVerifier(s, I, P);
        ...
        BigInt A = client.generateClientCredentials(s, I, P);
        if (A.compareTo(expect_A) != 0) {
          fail("wrong value of 'A'");
        }

        TestSRP6Server server = TestSRP6Server(
            group: SRP6StandardGroups.rfc5054_1024,
            v: v,
            digest: Digest("SHA-1"),
            random: random);

        BigInt B = server.generateServerCredentials();
        if (B.compareTo(expect_B) != 0) {
          fail("wrong value of 'B', expected $expect_B got $B");
        }

        BigInt u = SRP6Util.calculateU(Digest("SHA-1"), N, A, B);
        if (u.compareTo(expect_u) != 0) {
          fail("wrong value of 'u'");
        }

        BigInt clientS = client.calculateSecret(B);
        if (clientS.compareTo(expect_S) != 0) {
          fail("wrong value of 'S' (client)");
        }

        BigInt serverS = server.calculateSecret(A);
    ...
    // TODO add testWithRandomParams, requires DHParametersGenerator
    group("mutual verification:", () {
      test('testMutualVerificationWith1024', () {
        Uint8List I = Uint8List.fromList("username".codeUnits);
        Uint8List P = Uint8List.fromList("password".codeUnits);
        final key = Uint8List.fromList("keywithsixteenth".codeUnits);
        final keyParam = KeyParameter(key);
        final params = ParametersWithIV(keyParam, Uint8List(16));

        random.seed(params);
        Uint8List s = random.nextBytes(16);
...
class TestSRP6Client extends SRP6Client {
  BigInt a = BigInt.parse(
      "60975527035CF2AD1989806F0407210BC81EDC04E2762A56AFD529DDDA2D4393",
      radix: 16);

  TestSRP6Client({SRP6GroupParameters group, digest, random})
...
class TestSRP6Server extends SRP6Server {
  BigInt b = BigInt.parse(
      "E487CB59D31AC550471E81F00F6928E01DDA08E974A004F49E61F5D105284D20",
      radix: 16);

  TestSRP6Server({SRP6GroupParameters group, v, digest, random})
