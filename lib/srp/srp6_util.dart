...
class SRP6Util {
  static var _byteMask = BigInt.from(0xff);
  ...
  static BigInt calculateX(Digest digest, BigInt N, Uint8List salt,
    ...
    //TODO Make sure this conversion is valid
  ...
  static BigInt generatePrivateValue(
    ...
    BigInt min = BigInt.one << (minBits - 1);
    BigInt max = N - BigInt.one;
  ...
  static BigInt calculateKey(Digest digest, BigInt N, BigInt S) {
    ...
    Uint8List _S = getPadded(S, padLength);
  ...
  static BigInt hashPaddedTriplet(
    ...
    Uint8List n1Bytes = getPadded(n1, padLength);
    Uint8List n2Bytes = getPadded(n2, padLength);
    Uint8List n3Bytes = getPadded(n3, padLength);
  ...
  static Uint8List getPadded(BigInt n, int length) {
    Uint8List bs = encodeBigInt(n);
    if (bs.length < length) {
      Uint8List tmp = Uint8List(length);
  ...
  static BigInt hashPaddedPair(Digest digest, BigInt N, BigInt n1, BigInt n2) {
    int padLength = (N.bitLength + 7) ~/ 8;
